...
using static HarmonyXLib.AccessTools;

namespace HarmonyXLib.Internal.Util
{
	internal class ReflectionTools
	{
		...
		internal static AccessTools.FieldRef<T, F> FieldRefAccess<T, F>(FieldInfo fieldInfo, bool needCastclass)
		{
			...
			return (AccessTools.FieldRef<T, F>)dm.Generate().CreateDelegate(typeof(AccessTools.FieldRef<T, F>));
		}
		...
		internal static AccessTools.StructFieldRef<T, F> StructFieldRefAccess<T, F>(FieldInfo fieldInfo) where T : struct
		{
			...
			return (AccessTools.StructFieldRef<T, F>)dm.Generate().CreateDelegate(typeof(AccessTools.StructFieldRef<T, F>));
		}
		...
		internal static AccessTools.FieldRef<F> StaticFieldRefAccess<F>(FieldInfo fieldInfo)
		{
			...
			return (AccessTools.FieldRef<F>)dm.Generate().CreateDelegate(typeof(AccessTools.FieldRef<F>));
		}
